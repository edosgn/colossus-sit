...
export class NewTramiteLevantarPrendaComponent implements OnInit{ 
	...
	ngOnInit(){
		this.tramiteEspecifico = new TramiteEspecifico(null,45,this.tramiteGeneralId,null,null,null);
		let token = this._loginService.getToken();
		this._CasoService.showCasosTramite(token,45).subscribe(
				response => {
		...
		this._VarianteService.showVariantesTramite(token,45).subscribe(
				response => {
