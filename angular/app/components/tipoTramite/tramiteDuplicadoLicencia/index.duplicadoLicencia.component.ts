...
export class NewTramiteDuplicadoLicenciaComponent implements OnInit{ 
	...
	ngOnInit(){
		this.tramiteEspecifico = new TramiteEspecifico(null,12,this.tramiteGeneralId,null,null,null);
		let token = this._loginService.getToken();
		this._CasoService.showCasosTramite(token,12).subscribe(
				response => {
		...
		this._VarianteService.showVariantesTramite(token,12).subscribe(
				response => {
