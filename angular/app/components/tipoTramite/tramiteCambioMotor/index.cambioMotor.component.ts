...
export class NewTramiteCambioMotorComponent implements OnInit{ 
	...
	ngOnInit(){
		this.tramiteEspecifico = new TramiteEspecifico(null,24,this.tramiteGeneralId,null,null,null);
		let token = this._loginService.getToken();
		this._CasoService.showCasosTramite(token,24).subscribe(
				response => {
		...
		this._VarianteService.showVariantesTramite(token,24).subscribe(
				response => {
